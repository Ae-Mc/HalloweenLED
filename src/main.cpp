...
#define HUE_START \
    0  // начальный цвет огня (0 красный, 80 зелёный, 140 молния, 190 розовый)
...
#define COLOR_DEPTH 3  // цветовая глубина: 1, 2, 3 (в байтах)
...
// ВНИМАНИЕ! define настройки (ORDER_GRB и COLOR_DEBTH) делаются до подключения
// библиотеки!
#include <microLED.h>

LEDdata
    leds[NUM_LEDS];  // буфер ленты типа LEDdata (размер зависит от COLOR_DEBTH)
...
void setup() { strip.setBrightness(255); }
...
LEDdata getFireColor(int val) {
    // чем больше val, тем сильнее сдвигается цвет, падает насыщеность и растёт
    // яркость
    return mHSV(
